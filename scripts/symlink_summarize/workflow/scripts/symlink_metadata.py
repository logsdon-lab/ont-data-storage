...
def main():
    ...
    ap.add_argument(
        "-o",
        "--output_dir",
        default="/project/logsdon_shared/long_read_archive/{category}/{sample}/metadata",
    ...
    for file in glob.glob(input_dir.joinpath("*", "*", "*").as_posix()):
        file = pathlib.Path(file)
        is_dir = file.is_dir()
        # Ignore pod5 and fastq directories
        if file.name.startswith(("pod5", "fastq")) and is_dir:
            continue
        new_path = new_dir_path.joinpath(
            file.as_posix().replace(input_dir.as_posix() + "/", "")
        )
        ...
        try:
            if is_dir:
                ...
                new_path.parent.mkdir(exist_ok=True, parents=True)
                new_path.symlink_to(file, target_is_directory=True)
